    ...
    if (birthdayRule?.is_active) {
      try {
        ...
        const { data: contacts, error: contactsError } = await supabase
           .rpc('get_contacts_with_birthday', { p_month: month, p_day: day });
        ...
        for (const contact of birthdayContacts) {
            ...
            const { data: existing } = await supabase
              ...
              .eq('title', title)
              .gte('created_at', new Date(today.getFullYear(), today.getMonth(), today.getDate()).toISOString()) // Created today?
              .limit(1);
            ...
            if (!existing || existing.length === 0) {
              await supabase.from('tasks').insert({
                ...
                due_date: dueDate,
                description: 'Gerado automaticamente pela automação de aniversários.',
                is_completed: false
              });
              results.birthday++;
            }
      ...
      try {
        ...
        for (const days of checkPoints) {
          ...
          for (const purchase of (purchases || [])) {
             ...
             const contactName = Array.isArray(purchase.contacts) ? purchase.contacts[0].name : purchase.contacts.name;
    ...
    if (inactivityRule?.is_active) {
      try {
        ...
        const { data: inactiveContacts } = await supabase
          ...
          .or(`last_bid_date.lt.${ninetyDaysAgo.toISOString()},and(last_bid_date.is.null,created_at.lt.${ninetyDaysAgo.toISOString()})`);
        ...
        for (const contact of (inactiveContacts || [])) {
           ...
           if (!existing || existing.length === 0) {
              await supabase.from('tasks').insert({
                ...
                description: 'Cliente sem lances há mais de 90 dias. Tentar reengajamento.',
                is_completed: false
              });
              results.inactivity++;
           }
    ...
    if (lostBidRule?.is_active) {
      try {
        ...
        const { data: bids } = await supabase
          .from('bids')
          .select('id, contact_id, auction_id, lot_number, date, contacts(name)')
          .eq('date', dateStr);
        ...
        for (const bid of (bids || [])) {
          ...
          if (!purchase || purchase.length === 0) {
             const contactName = Array.isArray(bid.contacts) ? bid.contacts[0].name : bid.contacts.name;
